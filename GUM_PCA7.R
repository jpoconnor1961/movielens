...
mug50r_pq <- pca_mug50r$x[, 7] %*% t(pca_mug50r$rotation[, 7])
...
mug50r_pq_dfl <- pivot_longer(data = mug50r_pq_dfw, cols = 2:7082, names_to = "movieId", values_to = "p7q7")

mug50r_pq_dfl <- mug50r_pq_dfl %>% mutate(userId = as.integer(userId), movieId = as.integer(movieId))
rm(mug50r_pq_dfw)

mug50r_p7q7_edx <- edx %>% select(userId, movieId) %>% left_join(mug50r_pq_dfl, by = c('userId', 'movieId'))
mug50r_p7q7_edx$p7q7[is.na(mug50r_p7q7_edx$p7q7)] <- 0
mug50r_p7q7_val <- validation %>% select(userId, movieId) %>% left_join(mug50r_pq_dfl, by = c('userId', 'movieId'))
mug50r_p7q7_val$p7q7[is.na(mug50r_p7q7_val$p7q7)] <- 0
...
mug50r_pq_model_edx <- edx %>% select(userId, movieId, genreId) %>% 
  ...
  mutate(p5q5 = mug50r_p5q5_edx$p5q5) %>%
  mutate(p6q6 = mug50r_p6q6_edx$p6q6) %>%
  mutate(p7q7 = mug50r_p7q7_edx$p7q7) %>%
  mutate(y_hat = mu_hat + b_u_hat + b_i_hat + b_g_hat + p1q1 + p2q2 + p3q3 + p4q4 + p5q5 + p6q6 + p7q7) %>% 
  pull(y_hat)
mug50r_p7q7_rmse_edx <- RMSE(edx$rating, mug50r_pq_model_edx)
mug50r_p7q7_rmse_edx
[1] 0.8283959

rmse_results <- rmse_results %>% add_row(method = "GUM + P7Q7 model", RMSE = mug50r_p7q7_rmse_edx)
rmse_results
# A tibble: 11 × 2
   method            RMSE
   <chr>            <dbl>
 ...
 9 GUM + P5Q5 model 0.833
10 GUM + P6Q6 model 0.831
11 GUM + P7Q7 model 0.828
...
|GUM + P5Q5 model | 0.8330037|
|GUM + P6Q6 model | 0.8305945|
|GUM + P7Q7 model | 0.8283959|
...
mug50r_pq_model_val <- validation %>% select(userId, movieId, genreId) %>% 
  ...
  mutate(p5q5 = mug50r_p5q5_val$p5q5) %>%
  mutate(p6q6 = mug50r_p6q6_val$p6q6) %>%
  mutate(p7q7 = mug50r_p7q7_val$p7q7) %>%
  mutate(y_hat = mu_hat + b_u_hat + b_i_hat + b_g_hat + p1q1 + p2q2 + p3q3 + p4q4 + p5q5 + p6q6 + p7q7) %>% 
  pull(y_hat)
mug50r_p7q7_rmse_val <- RMSE(validation$rating, mug50r_pq_model_val)
mug50r_p7q7_rmse_val
[1] 0.8440613

rmse_testresults <- rmse_testresults %>% add_row(method = "GUM + P7Q7 model", RMSE = mug50r_p7q7_rmse_val)
rmse_testresults
# A tibble: 9 × 2
  method            RMSE
  <chr>            <dbl>
...
7 GUM + P5Q5 model 0.847
8 GUM + P6Q6 model 0.845
9 GUM + P7Q7 model 0.844
...
|GUM + P5Q5 model | 0.8466864|
|GUM + P6Q6 model | 0.8453096|
|GUM + P7Q7 model | 0.8440613|
