...
mug50r_pq <- pca_mug50r$x[, 5] %*% t(pca_mug50r$rotation[, 5])
...
mug50r_pq_dfl <- pivot_longer(data = mug50r_pq_dfw, cols = 2:7082, names_to = "movieId", values_to = "p5q5")

mug50r_pq_dfl <- mug50r_pq_dfl %>% mutate(userId = as.integer(userId), movieId = as.integer(movieId))
rm(mug50r_pq_dfw)

mug50r_p5q5_edx <- edx %>% select(userId, movieId) %>% left_join(mug50r_pq_dfl, by = c('userId', 'movieId'))
mug50r_p5q5_edx$p5q5[is.na(mug50r_p5q5_edx$p5q5)] <- 0
mug50r_p5q5_val <- validation %>% select(userId, movieId) %>% left_join(mug50r_pq_dfl, by = c('userId', 'movieId'))
mug50r_p5q5_val$p5q5[is.na(mug50r_p5q5_val$p5q5)] <- 0
...
mug50r_pq_model_edx <- edx %>% select(userId, movieId, genreId) %>% 
  ...
  mutate(y_hat = mu_hat + b_u_hat + b_i_hat + b_g_hat + p1q1 + p2q2 + p3q3 + p4q4 + p5q5) %>% 
  pull(y_hat)
mug50r_p5q5_rmse_edx <- RMSE(edx$rating, mug50r_pq_model_edx)
mug50r_p5q5_rmse_edx
[1] 0.8330037

rmse_results <- rmse_results %>% add_row(method = "GUM + P5Q5 model", RMSE = mug50r_p5q5_rmse_edx)
rmse_results
# A tibble: 9 × 2
  method            RMSE
  <chr>            <dbl>
...
mug50r_pq_model_val <- validation %>% select(userId, movieId, genreId) %>% 
  ...
  mutate(y_hat = mu_hat + b_u_hat + b_i_hat + b_g_hat + p1q1 + p2q2 + p3q3 + p4q4 + p5q5) %>% 
  pull(y_hat)
mug50r_p5q5_rmse_val <- RMSE(validation$rating, mug50r_pq_model_val)
mug50r_p5q5_rmse_val
[1] 0.8466864

rmse_testresults <- rmse_testresults %>% add_row(method = "GUM + P5Q5 model", RMSE = mug50r_p5q5_rmse_val)
rmse_testresults
# A tibble: 7 × 2
  method            RMSE
  <chr>            <dbl>
