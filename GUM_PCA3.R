...
mug50r_pq <- pca_mug50r$x[, 3] %*% t(pca_mug50r$rotation[, 3])
...
mug50r_pq_dfl <- pivot_longer(data = mug50r_pq_dfw, cols = 2:7082, names_to = "movieId", values_to = "p3q3")

mug50r_pq_dfl <- mug50r_pq_dfl %>% mutate(userId = as.integer(userId), movieId = as.integer(movieId))
rm(mug50r_pq_dfw)

mug50r_p3q3_edx <- edx %>% select(userId, movieId) %>% left_join(mug50r_pq_dfl, by = c('userId', 'movieId'))
mug50r_p3q3_edx$p3q3[is.na(mug50r_p3q3_edx$p3q3)] <- 0
mug50r_p3q3_val <- validation %>% select(userId, movieId) %>% left_join(mug50r_pq_dfl, by = c('userId', 'movieId'))
mug50r_p3q3_val$p3q3[is.na(mug50r_p3q3_val$p3q3)] <- 0
...
mug50r_pq_model_edx <- edx %>% select(userId, movieId, genreId) %>% 
  ...
  mutate(y_hat = mu_hat + b_u_hat + b_i_hat + b_g_hat + p1q1 + p2q2 + p3q3) %>% 
  pull(y_hat)
mug50r_p3q3_rmse_edx <- RMSE(edx$rating, mug50r_pq_model_edx)
mug50r_p3q3_rmse_edx
[1] 0.8390438

rmse_results <- rmse_results %>% add_row(method = "GUM + P3Q3 model", RMSE = mug50r_p3q3_rmse_edx)
rmse_results
# A tibble: 7 × 2
  method            RMSE
  <chr>            <dbl>
...
mug50r_pq_model_val <- validation %>% select(userId, movieId, genreId) %>% 
  ...
  mutate(y_hat = mu_hat + b_u_hat + b_i_hat + b_g_hat + p1q1 + p2q2 + p3q3) %>% 
  pull(y_hat)
mug50r_p3q3_rmse_val <- RMSE(validation$rating, mug50r_pq_model_val)
mug50r_p3q3_rmse_val
[1] 0.8505624

rmse_testresults <- rmse_testresults %>% add_row(method = "GUM + P3Q3 model", RMSE = mug50r_p3q3_rmse_val)
rmse_testresults
# A tibble: 5 × 2
  method            RMSE
  <chr>            <dbl>
